...
//        }
    ...
    void ART::recursive_remove_node(ARTKey key, ARTNode** node, ARTNode** parent, uint8_t depth, WriterTraceBlock* trace_block) {
        if(node == nullptr) {
            return;
        }
        ...
        {
            auto iter = alloc_iterator(*node);
            while (iter_is_valid(iter)) {
                auto cur_child = iter_get_current_ro(iter);
                if (IS_LEAF(cur_child)) {
                    assert(LEAF_RAW(cur_child)->depth == (*node)->depth);
                }
                iter_next_without_skip(iter);
            }
        ...
        if(child) {
            ...
            if((*child)->depth == depth + 1) {
                ...
                // path compression
                if(ARTKey::check_partial_match((*child)->prefix, key, (*child)->depth - 1)) {
        ...
        if((*node)->num_children == 0 && parent) {
            // remove the node from parent and then delete the node
            ...
            if((*parent)->type == NODE256) {
                auto node256 = (ARTNode_256*)(*parent);
                ARTLeaf* leaf_ptr = nullptr;
                for(int i = 0; i < 256; i++) {
                    if(node256->children[i] && leaf_ptr != LEAF_RAW(node256->children[i])) {
                        assert(GET_OFFSET(node256->children[i]) == 0);
            ...
            {
                auto iter = alloc_iterator(*parent);
                while (iter_is_valid(iter)) {
                    auto cur_child = iter_get_current_ro(iter);
                    if (IS_LEAF(cur_child)) {
                        assert(LEAF_RAW(cur_child)->depth == (*parent)->depth);
                    }
                    iter_next_without_skip(iter);
                }
                destroy_iterator(iter);
...
#endif
        std::tuple<std::vector<ARTNode*>*, ART*, ARTNode**> copy_res = copy_path(value, true, trace_block);
        auto path = std::get<0>(copy_res);
        if(path == nullptr) {
            assert(false);
            return ARTRemoveElemCopyRes{false, 0};
        }
        ART* copied_art = std::get<1>(copy_res);
        ARTNode** node = std::get<2>(copy_res);

        // set the ref_cnt & is_private
        for(auto path_node: *path) {
            resources->push_back(ARTResourceInfo{ARTResourceType::ART_Node_Copied, (void*)path_node});
        }
#ifndef NDEBUG
...
#endif
        if(res == CHILD_REMOVED && (*node)->num_children == 0) {
            recursive_remove_node(key, &copied_art->root, nullptr, copied_art->root->depth, trace_block);
        }
        if (res == NOT_FOUND) {
            for(uint8_t idx = 0; idx < path->size(); idx++) {
                resources->pop_back();
            }
        }
        delete path;
    ...
    ARTInsertElemBatchRes ART::insert_element_batch(const std::pair<RangeElement, RangeElement> *edges, Property_t** properties, uint64_t count, WriterTraceBlock* trace_block) {
//        std::cout << "count: " << count << ", first dest: " << edges[1].second << std::endl;
        ...
        delete ((ARTNode_4*) res_tree->root);    // delete the default root
        uint64_t inserted = batch_insert_copy(root, &res_tree->root, root->depth, insert_list->data(), properties, insert_list->size(), *resources, trace_block);
    ...
    ART* ART::set_property(uint64_t element, uint8_t property_id, Property_t property, WriterTraceBlock* trace_block) {
...
#endif
        std::tuple<std::vector<ARTNode*>*, ART*, ARTNode**> copy_res = copy_path(element, true, trace_block);
        auto path = std::get<0>(copy_res);
        if(path == nullptr) {
            assert(false);
            return nullptr;
        ...
        set_property_copy(node, ARTKey(element), element, property_id, property, *resources, trace_block);
